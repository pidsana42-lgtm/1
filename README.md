...
### 5. การสังเคราะห์คำถาม-คำตอบแบบมีกระบวนการคิดและการเรียกใช้เครื่องมือ (SFT QA Tool-Calling Generator)
รันสคริปต์นี้เพื่อสังเคราะห์คู่บทสนทนาแบบถาม-ตอบโต้ตอบกัน 2 รอบ (Multi-turn) โดยโมเดลจะจำลองกระบวนการคิดวิเคราะห์ในแท็ก `<think>...</think>` (Reasoning QA) และ**การเรียกใช้เครื่องมือค้นหาข้อมูลในคัมภีร์ (Tool Calling)** เพื่อนำข้อความจากตำราโบราณมาผูกคำทำนาย:
```bash
export HF_TOKEN="hf_xxxxxxxxxxxxxxxxxxxx"
bash run_qa_gen.sh --llm-model google/gemma-4-26B-A4B-it --batch-size 4
```
สคริปต์นี้จะช่วย:
- **Interactive Tool Calling Simulation**: โมเดลจะฝึกการตัดสินใจใช้เครื่องมือ `search_astrology_manual` โดยแปลงเจตนาของผู้ใช้เป็นคำสำคัญภาษาไทย (Keywords) เช่น `["ดวงตก", "แก้เคล็ด"]` เพื่อเรียกสืบค้นข้อมูลในคัมภีร์ จากนั้นจะส่งผลลัพธ์ข้อมูลกลับผ่านบทบาท `observation` เพื่อให้โมเดลประมวลผลคำตอบสุดท้ายตามหลักการในตำรา
- **Hugging Face RAG Fallback Download**: ไม่จำเป็นต้องรันขั้นตอนเตรียมข้อมูลก่อนเสมอไป! หากไม่พบไฟล์ `clean/rag_output.jsonl` ในเครื่อง สคริปต์จะดาวน์โหลดข้อมูล RAG Chunks ที่ล้างแล้วจาก `Phonsiri/astrology-dataset-clean` บน Hugging Face Hub ลงมาโดยอัตโนมัติ
- **Transformers-Only Execution**: รันการประมวลผลผ่าน Hugging Face Transformers โดยตรง ทำให้เปิดใช้งานและรันบนเครื่องคลาวด์เดี่ยวได้ง่ายโดยไม่ต้องเซ็ตอัปรัน vLLM Server
- **Backward Compatibility & Auto-Migration**: รองรับการ Resume รันต่อได้ หากตรวจพบบทสนทนา SFT เดิมบน Hugging Face ในรูปแบบคอลัมน์เก่า (`question1` / `response1`) ระบบจะอพยพโครงสร้างข้อมูลเดิม (Auto-migrate) ให้เป็นรูปแบบเครื่องมือจำลองอัตโนมัติทันที
- **ShareGPT Tool-use Output**: บันทึกคู่คำถามตอบลงเครื่องในรูปแบบ JSON ShareGPT Tool-use (`clean/sft_qa_thinking.json`) พร้อมฟิลด์ `tools` และ `conversations` ที่มีบทบาท `human`, `gpt` (ที่มี `tool_calls`) และ `observation` สำหรับนำไปป้อนระบบเทรนอย่าง LLaMA-Factory ได้ทันที
